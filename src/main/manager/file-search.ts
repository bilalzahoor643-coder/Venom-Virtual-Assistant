...
export async function indexFolder(folderPath: string): Promise<string> {
  try {
    ...
    await scanForIndexing(resolveSystemPath(folderPath))
      ...
      while (queue.length > 0 && foundFiles.length < 15) {
        ...
        for (const entry of entries) {
          ...
          if (isDir) {
            ...
            const cleanFileName = entry.name.toLowerCase().replace(/[,.\-_\s]+/g, '')
            const cleanPath = lowerPath.replace(/[,.\-_\s]+/g, '')
            const isMatch = searchParams.keywords.every((kw: string) => {
              const cleanKw = kw.replace(/[,.\-_\s]+/g, '')
              return cleanPath.includes(cleanKw) || cleanFileName.includes(cleanKw)
            })
            if (isMatch) {
...
export async function simpleFileSearch(query: string, searchDir?: string): Promise<string> {
  try {
    const rootDir = searchDir ? resolveSystemPath(searchDir) : os.homedir()
    const searchTerms = query.toLowerCase().replace(/[,.\-_\s]+/g, ' ').split(' ').filter(Boolean)
    ...
    while (queue.length > 0 && foundFiles.length < 20 && iterations < 5000) {
      const currentDir = queue.shift()
      if (!currentDir || visited.has(currentDir)) continue
      visited.add(currentDir)
      iterations++
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          const name = entry.name.toLowerCase()
          if (!name.startsWith('.') && !name.startsWith('$') && !IGNORE_FOLDERS.has(name)) {
            queue.push(fullPath)
          }
        } else if (entry.isFile()) {
          const cleanName = entry.name.toLowerCase().replace(/[,.\-_\s]+/g, '')
          const isMatch = searchTerms.every(term => cleanName.includes(term.replace(/[,.\-_\s]+/g, '')))
          if (isMatch) {
            foundFiles.push(fullPath)
          }
        }
    ...
    if (foundFiles.length > 0) {
      return `Found ${foundFiles.length} file(s):\n${foundFiles.join('\n')}`
    }
    return `No files found matching "${query}" in ${rootDir}`
  } catch (err: any) {
