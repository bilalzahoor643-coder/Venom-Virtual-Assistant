...
try {
  ...
  console.warn('[IRIS] nut-js not available in SmartDropZone')
}
...
export default function registerDropZoneControl(ipcMain: IpcMain) {
  ipcMain.handle('ghost-drag-and-drop', async (_event, { startX, startY, endX, endY }) => {
    try {
      if (!mouse || !Point || !Button) {
        return { success: false, error: 'Mouse control not available (nut-js not compiled)' }
      }
